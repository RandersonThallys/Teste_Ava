...
import 'package:teste_ava/app/modules/address/register_address/state/register_address_state.dart';

part 'register_address_store.g.dart';
...
abstract class _RegisterAddressStoreBase with Store {
  ...
  @observable
  RegisterAddressState stateRegister = RegisterAddressInitialState();
  ...
  register() async {
    ...
    result.when(
        (success) =>
            stateRegister = RegisterAddressSuccessState(address: success),
        (error) => stateRegister = RegisterAddressErrorState(
            messageError: error.message, address: stateRegister.address));
  }
